...
export function WidgetCustomizer({
  ...
  function handleDateRange(id: string, value: string) {
    runAction(() =>
      updateWidget({ id, overrideDateRange: value === "" ? null : value }),
    );
  }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              총 {widgets.length}개 · 순서/크기/기간을 조정할 수 있습니다.
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        {widgets.length === 0 ? (
          <div className="p-8 text-center text-sm text-slate-500">
            아직 추가된 위젯이 없습니다. 아래에서 원하는 프리셋을 추가하세요.
          </div>
        ) : (
              ...
              <li key={w.id} className="p-4 flex items-start gap-3 flex-wrap">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    value={w.width}
                    onChange={(e) =>
                      handleWidth(w.id, Number.parseInt(e.target.value, 10))
                    }
                    disabled={pending}
                  ...
                  >
                    ...
                    {DATE_RANGE_PRESETS.map((p) => (
                      <option key={p} value={p}>
                        {DATE_RANGE_LABEL[p as DateRangePreset]}
                      </option>
                    ))}
                  </select>
                </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {availableToAdd.length}개 프리셋을 추가할 수 있습니다.
          </p>
        </div>
