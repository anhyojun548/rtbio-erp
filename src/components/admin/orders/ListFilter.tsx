...
const STATUSES: { v: string; label: string }[] = [
  { v: "ALL", label: "전체" },
  { v: "DRAFT", label: "DRAFT" },
  { v: "SUBMITTED", label: "제출" },
  { v: "CONFIRMED", label: "확정" },
  { v: "HOLD", label: "보류" },
  { v: "REJECTED", label: "반려" },
  { v: "SHIPPING", label: "출고중" },
...
export function OrderListFilter({
  ...
  function apply(
    overrides: Partial<{ q: string; status: string; from: string; to: string }> = {},
  ) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 flex flex-wrap items-end gap-3">
      ...
      <div>
        <label className="block text-xs text-slate-500 mb-1">주문일 From</label>
        <input
          type="date"
          value={from}
