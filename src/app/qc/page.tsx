    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface rounded shadow-sm border border-border">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <Link href="/qc/shipments" className="text-tiny text-primary hover:underline">
              보드 열기 →
            </Link>
          </div>
          {columns.length === 0 ? (
              ...
              {columns.map((col) => (
                <li key={col.id} className="flex items-center px-5 py-3 text-caption">
                  <span className="flex-1 font-semibold text-ink">
                    {col.label}
                    {col.isTerminal && (
                      <span className="ml-2 text-tiny px-1.5 py-0.5 rounded-xs bg-success-light text-success font-semibold">
                        terminal
                      </span>
                    )}
                  </span>
        ...
        <div className="bg-surface rounded shadow-sm border border-border">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <Link href="/qc/alerts" className="text-tiny text-primary hover:underline">
              전체 →
            </Link>
          </div>
          {topAlerts.length === 0 ? (
      ...
      <section className="bg-surface rounded shadow-sm border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          ...
          <Link href="/qc/expiry" className="text-tiny text-primary hover:underline">
            전체 →
          </Link>
        </div>
        {expiring.length === 0 ? (
