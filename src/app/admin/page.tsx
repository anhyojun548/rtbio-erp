    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="text-caption text-ink-secondary mt-1">
          ...
          <code className="font-mono bg-canvas px-2 py-0.5 rounded-xs text-tiny">
            {user.tenantCode ?? "-"}
          </code>
        </p>
      </header>
      ...
      <section className="bg-surface rounded shadow-sm border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-h3 m-0 flex items-center gap-2">
            🚨 재고 임계치 알럼
            {alertTotal > 0 && (
              <span className="text-tiny font-normal text-ink-secondary">
                ({alertCounts.OUT > 0 && <span className="text-danger font-semibold">품절 {alertCounts.OUT}</span>}
                {alertCounts.OUT > 0 && alertCounts.LOW > 0 && " · "}
                {alertCounts.LOW > 0 && <span className="text-warning font-semibold">부족 {alertCounts.LOW}</span>})
              </span>
            )}
          </h2>
          <Link href="/admin/alerts/stock" className="text-tiny text-primary hover:underline">
            전체 보기 →
          </Link>
        </div>
        {topAlerts.length === 0 ? (
          <div className="p-8 text-center text-caption text-success bg-success-light/30">
            ✨ 모든 활성 사이즈가 정상 재고입니다.
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-border">
              {topAlerts.map((r) => (
                <tr key={r.sizeId} className="hover:bg-canvas">
                  ...
                  <td className="px-4 py-2.5">
                    <Link href={`/admin/products/${r.productId}`} className="text-primary hover:underline">
                      {r.productName}
                    </Link>
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-h3 m-0 flex items-center gap-2">
            ...
            {expiringContracts.length > 0 && (
              <span className="text-tiny font-normal text-warning">
                ({expiringContracts.length}건)
              </span>
            )}
          </h2>
          <Link href="/admin/contracts?status=ENDING_SOON" className="text-tiny text-primary hover:underline">
            전체 보기 →
          </Link>
        </div>
        {expiringContracts.length === 0 ? (
            ...
            <tbody className="divide-y divide-border">
              {expiringContracts.slice(0, 5).map((c) => (
                <tr key={c.id} className="hover:bg-canvas">
                  ...
                  <td className="px-4 py-2.5">
                    <Link href={`/admin/contracts/${c.id}`} className="text-ink hover:underline">
                      {c.title}
                    </Link>
                  </td>
                  <td className="px-4 py-2.5 text-primary">{c.client.name}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-tiny text-ink-secondary">
                    {c.endDate?.toISOString().slice(0, 10) ?? "—"}
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-semibold text-warning">
