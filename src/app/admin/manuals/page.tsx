    ...
    <div className="space-y-6">
      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-tiny text-ink-secondary font-semibold">카테고리:</span> {categories.map((c) => (
            <span key={c.category} className="px-2 py-0.5 rounded-full text-tiny bg-canvas border border-border text-ink-secondary"> {c.category} ({c.count})
            </span> ))}
        </div> )}

      {/* 문서 목록 (3구역) */}
      {(["MANUAL", "PROCEDURE", "FORM"] as const).map((k) => byKind[k].length > 0 && (
          <section key={k} className="bg-surface border border-border rounded">
            <h2 className="text-h3 px-5 py-3 border-b border-border m-0"> {QDOC_KIND_LABEL[k]} <span className="text-tiny text-ink-muted ml-2">{byKind[k].length}건</span>
            </h2>
            <ul className="divide-y divide-border"> {byKind[k].map((d) => (
                <li key={d.id} className="flex items-center px-5 py-3 text-caption hover:bg-canvas transition">
                  ...
                  <span className="flex-1 text-ink">{d.title}</span> {d.category && <span className="text-tiny text-ink-muted mr-3">{d.category}</span>}
                  {d.revision && <span className="text-tiny font-mono text-ink-muted">Rev.{d.revision}</span>}
                  {d.filePath && (
                    <a href={d.filePath} target="_blank" className="ml-3 text-tiny text-primary hover:underline"> 다운로드
                    </a> )}
                </li> ))}
            </ul>
          </section> )
      ...
      {docs.length === 0 && (
        <div className="bg-surface border border-border rounded p-12 text-center text-ink-muted"> 등록된 문서가 없습니다.
        </div> )}
    </div> );
}
