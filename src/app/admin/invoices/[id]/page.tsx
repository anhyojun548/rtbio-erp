...
export default async function InvoiceDetailPage({
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-display m-0 font-mono"> {displayNumber}
            </h1>
            <span
          ...
          <p className="text-caption text-ink-secondary mt-1"> 발행일 {new Date(inv.issueDate).toLocaleDateString("ko-KR")}
            {inv.dueDate &&
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6"> {/* 좌 — 거래처 / 요약 */}
        <section className="lg:col-span-2 rounded-lg border border-slate-200 bg-white p-5 space-y-3">
          ...
          <InfoRow
            ...
            value={
              <span>
                ...
                <span className="font-mono text-xs text-slate-500"> ({inv.client.code})
                </span>
              </span> }
          />
          ...
          <div className="pt-3 mt-3 border-t border-slate-100 space-y-2">
            <h3 className="font-semibold text-slate-900 text-sm">연결 주문</h3> {inv.order ? (
              <InfoRow
                label="주문번호"
                value={
                  <Link
          ...
          <div className="pt-3 mt-3 border-t border-slate-100 space-y-2">
            ...
            <InfoRow
              label="합계"
              value={
                <strong> {Number(inv.totalAmount).toLocaleString("ko-KR")}원
                </strong> }
            />
          </div>
        </section> {/* 우 — DRAFT 편집 또는 비고 표시 */}
        <section className="lg:col-span-3 space-y-4"> {isDraft ? (
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-5">
              ...
              <p className="text-sm text-slate-700 whitespace-pre-wrap"> {inv.note ?? "(없음)"}
              </p>
            </div> )}
      ...
      <section className="rounded-lg border border-slate-200 bg-white">
        <header className="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500"> 주문 스냅샷 — 수정 불가
          </span>
        </header>
        <table className="w-full text-sm">
          ...
          <tbody> {inv.items.map((it, idx) => (
              <tr key={it.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2 text-right tabular-nums"> {it.quantity.toLocaleString("ko-KR")}
                </td>
                <td className="px-4 py-2 text-right tabular-nums text-slate-700"> {Number(it.unitPrice).toLocaleString("ko-KR")}
                </td>
                <td className="px-4 py-2 text-right tabular-nums font-medium"> {Number(it.amount).toLocaleString("ko-KR")}
                </td>
              </tr> ))}
          </tbody>
          <tfoot className="bg-slate-50 border-t border-slate-200">
            <tr>
              <td colSpan={4} className="px-4 py-2 text-right text-slate-600"> 공급가액
              </td>
              <td className="px-4 py-2 text-right tabular-nums"> {Number(inv.supplyAmount).toLocaleString("ko-KR")}
              </td>
            </tr>
            <tr>
              <td colSpan={4} className="px-4 py-2 text-right text-slate-600"> 부가세 (10%)
              </td>
              <td className="px-4 py-2 text-right tabular-nums"> {Number(inv.vatAmount).toLocaleString("ko-KR")}
              </td>
            </tr>
            <tr className="bg-slate-900 text-white">
              <td colSpan={4} className="px-4 py-3 text-right font-semibold"> 합계 금액
              </td>
              <td className="px-4 py-3 text-right tabular-nums font-bold"> {Number(inv.totalAmount).toLocaleString("ko-KR")} 원
              </td>
            </tr>
          </tfoot>
        </table>
