...
> **Flowise 연동 가이드**: 토큰 인증(`Authorization: Bearer <WIDGET_API_TOKEN>`) + `forUser`(대상 사용자 이메일) 기반 외부 에이전트 통합은 [dashboard-widget-api.md](./dashboard-widget-api.md) 참고 (도구 3종 사용법 · WidgetSpec 레퍼런스 · few-shot · Flowise 시스템 프롬프트). 토큰은 모든 `GET /api/*` + `POST /api/dashboard/widgets/spec` 만 허용(그 외 쓰기 403), `svc-integration` ADMIN 서비스 계정으로 동작.
