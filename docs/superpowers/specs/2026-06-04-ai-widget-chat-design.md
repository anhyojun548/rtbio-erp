...
- **세션 인증**: 인앱은 로그인 사용자다. 기존 위젯 토큰(`WIDGET_API_TOKEN`)은 **외부 Flowise→우리** 방향 전용이므로 **인앱 라우트엔 쓰지 않는다.** 라우트는 **NextAuth 세션**으로 보호한다 — 형제 저장 라우트 `POST /api/dashboard/widgets/spec` 와 **동일한 인증 수준**(세션 = 자기 대시보드). `/ai` 는 정적 prefab 템플릿만 반환(민감 데이터 누출 없음)하고 실제 저장은 `/spec` 가 담당하므로, **추가 롤 게이트는 두지 않는다(저장 경로와 일관).** 더 강한 게이트가 필요해지면 `/spec` 와 함께 올린다. (구현 확정: 세션-only.)
